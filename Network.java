...
public class Network {
    ...
    public boolean addFollowee(String name1, String name2) {
        if (this.getUser(name2) == null || this.getUser(name1) == null
    ...
    public String recommendWhoToFollow(String name) {
        ...
        User mostMutualFollowees = null, otherUser = this.getUser(name);
        for (int i = 1; i < userCount; i++) {
            User currentUser = users[i];
            if ((!currentUser.getName().equals(name)) && (mostMutualFollowees == null
                    || mostMutualFollowees.countMutual(otherUser) < currentUser.countMutual(otherUser))) {
                mostMutualFollowees = currentUser;
            }
        }
        return mostMutualFollowees.getName();
    }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 1; i < userCount; i++) {
            User currentUser = users[i];
            if (followeeCount(currentUser.getName()) > followeeCount(mostPopular.getName())) {
